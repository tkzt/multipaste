...
#[tauri::command]
pub fn update_auto_start(auto_start: bool, app_handle: AppHandle, config: State<Mutex<Config>>) -> bool {
...
#[tauri::command]
pub fn update_max_items(max_items: u64, app_handle: AppHandle, config: State<Mutex<Config>>, store: State<Arc<RecordStore>>) -> bool {
    if let Ok(mut config) = config.lock() {
...
fn dump_config(config_path: &PathBuf, config: &Config) -> Result<(), Box<dyn std::error::Error>> {
  std::fs::write(config_path, serde_json::to_string(config).unwrap_or(DEFAULT_CONFIG_STR.to_string()))?;
  Ok(())
}
